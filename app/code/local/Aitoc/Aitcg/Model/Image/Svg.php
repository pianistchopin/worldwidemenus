    ...
    public function getSvgData($header = true, $xlink = false)
    {
        ...
        $svg = ($header ? $this->getLayer('header') : '').
            '<svg '.
               ...
               ($this->_maskApplied ? $this->getLayer('mask') : '').( (!$this->_maskApplied || !$this->allowToPrint('mask')) && $this->allowToPrint('bg') ? $this->_backgroundData['mask_xml'] : '').
            '</svg>';
        /**  $this->getLayer('pdf_bg').
        $this->_generateLayerSvg('bottom').
    ...
    */
    ...
    public function normalizeMask()
    {
        ...
        foreach($arrTable[0] as $key => $img)
        {
            ...
            if(strpos($img, '1x1_black.png') !== false) {
				unset($arrTable[0][$key]);
            } else {
	            $arrTable[0][ $key ] = str_replace( 'class="Mask"', 'class="Mask" opacity="0.4"', $img );
            }
        }
    ...
    protected function _generateLayerSvg($key)
    {
        ...
        return '<g transform="translate('.$offset_x.','.$offset_y.')">'.$data.'</g>';
    }
    ...
    public function resetMaskForPng()
    {
	    ...
	    $data = preg_replace('/opacity="(.*).4"/U','opacity="0.4"', $data);
