...
class Aitoc_Aitcg_Model_Rewrite_Catalog_Product_Option extends Mage_Catalog_Model_Product_Option
{
    ...
    const ONE_OFF_COST_MESSAGE="(One of Cost)";
    /**
    ...
    public function getTitle(){
        ...
        else if($this->getIsOneOfCost())
        {
            $contain=!!preg_match('#\\b' . preg_quote(self::ONE_OFF_COST_MESSAGE, '#') . '\\b#i',$title);
            if(!$contain)
            {
                return $title."  ".self::ONE_OFF_COST_MESSAGE." ";
            }
        }
        return $title;
    }
