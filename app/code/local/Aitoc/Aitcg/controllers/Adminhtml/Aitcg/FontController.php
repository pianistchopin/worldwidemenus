...
class Aitoc_Aitcg_Adminhtml_Aitcg_FontController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
                Mage::helper('aitcg')->__('Aitoc Custom Product Preview Fonts'), Mage::helper('aitcg')
                ->__('Aitoc Custom Product Preview Fonts')
            );
    ...
    public function saveAction()
    {
        if ($this->getRequest()->getPost()) {
            try {
                ...
                if (isset($postData['font_family_id'])) {

                    if ($postData['font_family_id']) {
                        $fontModel->setFontFamilyId($postData['font_family_id']);
                    } else {
                        $newFontFamilyName = $postData['new_font_family'];

                        $fontFamilyModel = Mage::getModel('aitcg/font_family');;

                        $fontFamilyModel->setTitle($newFontFamilyName);
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('catalog/aitcg/fonts');
    }
}