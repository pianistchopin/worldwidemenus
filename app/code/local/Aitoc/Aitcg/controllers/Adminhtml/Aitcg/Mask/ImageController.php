...
class Aitoc_Aitcg_Adminhtml_Aitcg_Mask_ImageController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function createPreviewImage($maskmodel){
        if (!empty($maskmodel) && !empty($maskmodel->getFilename()) && file_exists($maskmodel->getImagesPath() . $maskmodel->getFilename())) {
            $ext = pathinfo($maskmodel->getFilename(), PATHINFO_EXTENSION);
            $filename = Mage::helper('aitcg')->getSlug($maskmodel->getName()) . '.' . $ext;
            $thumb = new Varien_Image($maskmodel->getImagesPath() . $maskmodel->getFilename());
            $thumb->open();
            ...
            $thumb->resize(135);
            $thumb->save($maskmodel->getImagesPath() . 'preview' . DS . $filename);
        }
    }
    public function saveAction()
    {
        if ( $this->getRequest()->getPost() ) {
            try {
                ...
                if(isset($postData['filename'])) {
                    $this->createPreviewImage($imageModel);
                }
                else{
                    $maskmodel = Mage::getModel('aitcg/mask')->load($this->getRequest()->getParam('imgid'));
                    $this->createPreviewImage($maskmodel);
                }
                /*
                if($postData['preview']) {
                    $collection = Mage::getResourceModel('aitcg/mask_collection')->addFieldToFilter('name', array('like' => $postData['name']))->
                    addFieldToFilter('id', array('neq' => $this->getRequest()->getParam('imgid')));

                    if($collection->getSize())
                    {
                        $collection->massUpdate(array('preview' => 0));
                    }
                    if(isset($postData['filename'])) {
                        $this->createPreviewImage($imageModel);
                    }
                    ...
                    //Mage::helper('aitcg/image')->createPreviewImage($maskmodel);
                }*/
                $imageModel->save();
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('catalog/aitcg/mask_categories');
    }
