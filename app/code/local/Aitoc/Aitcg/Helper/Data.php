...
class Aitoc_Aitcg_Helper_Data extends Aitoc_Aitcg_Helper_Abstract
{
    ...
    protected static $_specified_category=array('moscow','madrid');
    /**
  ...
  // const VYA_INTEGRATION_GALLERY_PREVIEW_SIZE = 295;
    ...
    public function getOneOffCostLabel(){
        return __("Please use one size printing plate across all items- one plate charge.");
    }
    ...
    public function getIsSpecifiedCategory(){
        $product=Mage::registry('current_product');
        if ($product && $product->getId()) {
            ...
            foreach ($categoryIds as $category_id) {
                ...
                if ($_cat->getData('name')) {
                    $cat_name=strtolower(trim($_cat->getData('name')));
                    if ($cat_name  && in_array($cat_name,self::$_specified_category )) {
                        return 1;
                    }
    ...
    public function checkIsMirroringSpreadCategory($catories) {
	    $category = Mage::registry('current_category')->getName();
		if(!empty( $catories)) {
			foreach ($catories as $cat) {
				if (strpos(strtolower($category), strtolower($cat)) !== false) {
					return 1; break;
				}
			}
		}
    ...
    public function checkIsCategory($cat){
        $result=array();
        $product=Mage::registry('current_product');
        if ($product && $product->getId()) {
            $categoryIds = $product->getCategoryIds();
            ...
            foreach ($categoryIds as $category_id) {
                ...
                if ($_cat->getData('name')) {
                    $cat_name=strtolower(trim($_cat->getData('name')));
                    if ($cat_name  && in_array($cat_name,$cat )) {
						if($cat_name == 'bill presenters') {
							$cat_name = 'bill_presenters';
						}
                    	$result[$cat_name]=1;
                    }
