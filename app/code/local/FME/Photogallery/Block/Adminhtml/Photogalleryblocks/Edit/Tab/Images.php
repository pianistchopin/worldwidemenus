 ...
 */
class FME_Photogallery_Block_Adminhtml_Photogalleryblocks_Edit_Tab_Images extends Mage_Adminhtml_Block_Widget
{
    ...
    {
           $test = Mage::registry('photogallery_blockimage');
           $blockimage_data = $test->getData();
           return $blockimage_data; 
    }
    ...
    public function getImagesJson()
    {
        ...
        if(is_array($polje)) {
            ...
            if(count($value['images'])>0) {
                // $image['img_description']=null;
                // $image['alt_text']=null;
                    foreach ($value['images'] as &$image) {
                        $image['url'] = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'gallery/galleryimages'.$image['blockimage_name']; 
                        $image['value_id'] = $image['blockimage_id'];
                        unset($image['blockimage_id']);
                        $image['file'] = $image['blockimage_name'];
                        unset($image['blockimage_name']);
                        $image['label'] = $image['blockimage_label'];
                        unset($image['blockimage_label']);
                        $image['alt_text'] = $image['alt_text'];
                        $image['description'] = $image['img_description'];
                        unset($image['img_description']);
                        $image['position'] = $image['blockimage_order'];
                        unset($image['blockimage_order']);
                    }
