...
class MageWorx_CustomOptions_Model_Mysql4_Relation extends Mage_Core_Model_Mysql4_Abstract {
    ...
    protected function _construct() {
        $this->_init('mageworx_customoptions/relation', 'id');
    }
    ...
    public function getGroupIds($productId, $onlyActive = false) {
        ...
        if ($onlyActive === true) {
            $gruopsIds = Mage::getSingleton('mageworx_customoptions/group')->getActiveGruopsIds();
            if ($gruopsIds) {
    ...
    public function setOptionsStatus($optionId, $status = 1) {
        $this->_getWriteAdapter()->update(
                $this->getTable('mageworx_customoptions/option_view_mode'),
