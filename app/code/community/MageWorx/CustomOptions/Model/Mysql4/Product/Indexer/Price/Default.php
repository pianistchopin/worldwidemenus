...
class MageWorx_CustomOptions_Model_Mysql4_Product_Indexer_Price_Default extends MageWorx_CustomOptions_Model_Mysql4_Product_Indexer_Price_Default_Abstract
{
    protected function _applyCustomOption() {
        $helper = Mage::helper('mageworx_customoptions');
        if (!$helper->isEnabled()
        	|| version_compare($helper->getMagetoVersion(), '1.4.0', '<')
        	|| (string)Mage::getConfig()->getModuleConfig('Innoexts_AdvancedPricing')->active == 'true'
        	|| (string)Mage::getConfig()->getModuleConfig('Innoexts_StorePricing')->active == 'true'
    ...
    protected function _applyCustomOption1700() {
        ...
        $minPrice = $write->getCheckSql("FIND_IN_SET(i.customer_group_id, o.customer_groups) OR i.customer_group_id = o.customer_groups", $minPrice, 'i.orig_price'); 
    ...
    protected function _applyCustomOption1620() {
        ...
        if ($this->useIdxTable() && (version_compare(Mage::helper('mageworx_customoptions')->getMagetoVersion(), '1.6.1', '<') || version_compare(Mage::helper('mageworx_customoptions')->getMagetoVersion(), '1.6.1', '=') && $this->_allowTableChanges)) {
    ...
    protected function _applyCustomOption1510() {
        ...
        $minPrice = new Zend_Db_Expr("IF(FIND_IN_SET(i.customer_group_id, o.customer_groups) OR i.customer_group_id = o.customer_groups,"
            . "IF(o.is_require, MIN(IF(IF(otps.option_type_price_id>0, otps.price_type, "
            . "otpd.price_type)='fixed', IF(otps.option_type_price_id>0, otps.price, otpd.price), "
            . "ROUND(i.price * (IF(otps.option_type_price_id>0, otps.price, otpd.price) / 100), 4))), 0), i.orig_price)");
        ...
        if (version_compare(Mage::helper('mageworx_customoptions')->getMagetoVersion(), '1.4.2', '<') || $this->useIdxTable()) {
