...
class MageWorx_CustomOptions_Model_Catalog_Product_Option extends Mage_Catalog_Model_Product_Option {
    ...
    const PRICE_TYPE_OPTIONS_PERCENT = 'optperc';
    const PRICE_TYPE_FIXED_PER_CHARACTER = 'fixchar';

    const OPTION_VIEWMODE_DISABLE = 0;
    const OPTION_VIEWMODE_VISIBLE = 1;
    const OPTION_VIEWMODE_HIDDEN = 2;
    const OPTION_VIEWMODE_BACKEND = 3;
    const OPTION_VIEWMODE_ADMINONLY = 4;

    protected function _construct() {
        parent::_construct();
        $this->_init('mageworx_customoptions/product_option');
    }    
    ...
    public function _prepareOptions($options, $templateId, $groupIsActive = 2) {
        ...
        foreach ($options as $key=>$op) {                
            ...
            if (isset($op['id']) && ($op['type']=='field' || $op['type']=='area') && !isset($op['image_path'])) {
                if (Mage::helper('mageworx_customoptions')->isCustomOptionsFile($templateId, $op['id'])) {
                ...
                if (isset($op['values']) && is_array($op['values'])) {
                    ...
                    foreach ($op['values'] as $k=>$value) {
                        ...
                        if (isset($value['images']) && is_array($value['images']) && count($value['images'])>0) {
                            ...
                            $result = Mage::helper('mageworx_customoptions')->getCheckImgPath($value['image_path']);
                            if ($result) {
                        ...
                        if (array_key_exists('extra', $value) && substr($value['extra'], 0, 1)=='i') {
    ...
    public function removeProductOptionsAndRelationByGroup($templateId) {
        ...
        if ($templateId>0) {
            ...
            Mage::getResourceSingleton('mageworx_customoptions/relation')->deleteGroup($templateId); // just in case
        }
        return $result;
    }
    ...
    public function saveProductOptions($newOptions, array $prevOptions, array $productIds, Varien_Object $group, $prevGroupIsActive = 1, $place = 'apo', $prevStoreOptionsData = array()) {
        if (isset($productIds) && is_array($productIds) && count($productIds)>0) {
            $relation = Mage::getResourceSingleton('mageworx_customoptions/relation');
            ...
            $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_relation')->where('group_id = ' . $templateId . $condition);
            ...
            if (isset($newOptions) && is_array($newOptions)) {
                ...
                $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_group_store')->where('group_id = '.$templateId);
...
//                exit;
                ...
                foreach ($productIds as $productId) {                
                        ...
                        foreach ($optionRelations[$productId] as $optionId=>$prevOption) {
                            ...
                            if (isset($options['IGI'.$prevOption->getInGroupId()]) && (!isset($options['IGI'.$prevOption->getInGroupId()]['is_delete']) || $options['IGI'.$prevOption->getInGroupId()]['is_delete']!=1)) {
                                ...
                                if (isset($prevOption['option_id'])) {
                                    ...
                                    $connection->delete($tablePrefix . 'mageworx_custom_options_option_view_mode', 'option_id = ' . $prevOption['option_id']);
                                    $connection->delete($tablePrefix . 'mageworx_custom_options_option_description', 'option_id = ' . $prevOption['option_id']);
                                    $connection->delete($tablePrefix . 'mageworx_custom_options_option_default', 'option_id = ' . $prevOption['option_id']);
                                    $connection->delete($tablePrefix . 'mageworx_custom_options_relation', 'group_id = '. $templateId .' AND product_id = '.$productId.' AND option_id = ' . $prevOption['option_id']);
                                }
                            }                            
                        }
                    }                                        
                    ...
                    foreach ($options as $option) {
                        ...
                        if ($group->getOnlyUpdate() && isset($optionRelations[$productId]) ) continue;
                        ...
                        $connection->insert($tablePrefix . 'mageworx_custom_options_relation', $optionRelation);
    ...
    public function saveOption($productId, $option, $optionId = 0, $storeId = 0, $type = '') {
        ...
        if ($storeId==0) {
            ...
            if (isset($option['show_swatch_title'])) $optionData['show_swatch_title'] = $option['show_swatch_title'];
        ...
        if (isset($option['view_mode'])) {
            ...
            $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_view_mode', array('view_mode'))->where('option_id = '.$optionId.' AND `store_id` = '.$storeId);
            $updateViewModeFlag = $connection->fetchOne($select);
            if ($option['view_mode']!==$updateViewModeFlag) {
                if ($updateViewModeFlag!==false) {
                    $connection->update($tablePrefix . 'mageworx_custom_options_option_view_mode', $optionMode, 'option_id = '.$optionId.' AND `store_id` = '.$storeId);
                } else {
                    $connection->insert($tablePrefix . 'mageworx_custom_options_option_view_mode', $optionMode);
                }
            }    
        } elseif ($storeId>0 || isset($option['view_mode'])) {
            $connection->delete($tablePrefix . 'mageworx_custom_options_option_view_mode', 'option_id = '.$optionId.' AND store_id = '.$storeId);
        }
        ...
        if (isset($option['description']) && $option['description']!='') {
            ...
            $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_description', array('description'))->where('option_id = '.$optionId.' AND `store_id` = '.$storeId);
            $updateDescriptionFlag = $connection->fetchOne($select);
            if ($option['description']!==$updateDescriptionFlag) {
                if ($updateDescriptionFlag) {
                    $connection->update($tablePrefix . 'mageworx_custom_options_option_description', $optionDesc, 'option_id = '.$optionId.' AND `store_id` = '.$storeId);
                } else {
                    $connection->insert($tablePrefix . 'mageworx_custom_options_option_description', $optionDesc);
                }
            }    
        } elseif ($storeId>0 || isset($option['description']) && $option['description']=='') {
            $connection->delete($tablePrefix . 'mageworx_custom_options_option_description', 'option_id = '.$optionId.' AND store_id = '.$storeId);
        }
        ...
        if (isset($option['default_text']) && $option['default_text']!='') {
            ...
            $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_default', array('default_text'))->where('option_id = '.$optionId.' AND `store_id` = '.$storeId);
            $updateDefaultTextFlag = $connection->fetchOne($select);
            if ($option['default_text']!==$updateDefaultTextFlag) {
                if ($updateDefaultTextFlag) {
                    $connection->update($tablePrefix . 'mageworx_custom_options_option_default', $optionDef, 'option_id = '.$optionId.' AND `store_id` = '.$storeId);
                } else {
                    $connection->insert($tablePrefix . 'mageworx_custom_options_option_default', $optionDef);
                }
            }    
        } elseif ($storeId>0 || (isset($option['default_text']) && $option['default_text']=='')) {
            $connection->delete($tablePrefix . 'mageworx_custom_options_option_default', 'option_id = '.$optionId.' AND store_id = '.$storeId);
        }                
    ...
    public function saveOptionValue($optionId, $value, $prevValue, $newValue, $optionTypeId = 0, $storeId = 0) {
        ...
        if ($storeId==0) {
            ...
            if (isset($value['upc'])) $optionValue['upc'] = $value['upc'];
                    ...
                    if (isset($prevValue['tiers'])) {
                        foreach ($prevValue['tiers'] as $key=>$tierValue) {
                            if (!isset($newValue['tiers'][$key])) {
                                $connection->delete($tablePrefix . 'mageworx_custom_options_option_type_tier_price',
    ...
    public function saveOptionValueImage($optionTypeId, $imageFile, $sort, $isUpdate) {
        ...
        $optionTypeImageId = 0;
        if ($isUpdate) {
            $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_type_image', array('option_type_image_id'))->where($connection->quoteInto('option_type_id = '. $optionTypeId .' AND image_file = ?', $imageFile));
        ...
        if ($optionTypeImageId>0) {
            $connection->update($tablePrefix . 'mageworx_custom_options_option_type_image', $optionTypeImage, 'option_type_image_id = ' . $optionTypeImageId);
        } else {
            $connection->insert($tablePrefix . 'mageworx_custom_options_option_type_image', $optionTypeImage);
        }
    }
    ...
    public function saveOptionValueSpecial($optionTypePriceId, $prevCustomerGroupId, $specialValue) {
        ...
        if (!is_null($prevCustomerGroupId)) {
            $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_type_special_price', array('option_type_special_price_id'))
        ...
        if ($optionTypeSpecialPriceId>0) {
            $connection->update($tablePrefix . 'mageworx_custom_options_option_type_special_price', $optionTypeSpecialPrice, 'option_type_special_price_id = '.$optionTypeSpecialPriceId);
        } else {
            try {
                $connection->insert($tablePrefix . 'mageworx_custom_options_option_type_special_price', $optionTypeSpecialPrice);
            } catch (Exception $e) {}
        }
    }
    ...
    public function saveOptionValueTier($optionTypePriceId, $prevQty, $prevCustomerGroupId, $tierValue) {
        ...
        if ($prevQty>0) {
            $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_type_tier_price', array('option_type_tier_price_id'))
        ...
        if ($optionTypeTierPriceId>0) {
            $connection->update($tablePrefix . 'mageworx_custom_options_option_type_tier_price', $optionTypeTierPrice, 'option_type_tier_price_id = '.$optionTypeTierPriceId);
        } else {
            $connection->insert($tablePrefix . 'mageworx_custom_options_option_type_tier_price', $optionTypeTierPrice);
        }
    }
    ...
    public function updateProductFlags($productId, $absolutePrice = 0, $absoluteWeight = 0, $skuPolicy = 0) {
        ...
        $select = $connection->select()
                ...
                ->join(array('view_mode_tbl' => $tablePrefix . 'mageworx_custom_options_option_view_mode'), 'view_mode_tbl.option_id = option_tbl.option_id AND view_mode_tbl.store_id=0', '')
    ...
    private function _removeOptionViewMode($id) {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
        $connection->delete($tablePrefix . 'mageworx_custom_options_option_view_mode', 'option_id = ' . $id);
    }
    
    private function _removeOptionDescription($id) {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
        $connection->delete($tablePrefix . 'mageworx_custom_options_option_description', 'option_id = ' . $id);
    }
    
    private function _removeOptionDefaultText($id) {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
        $connection->delete($tablePrefix . 'mageworx_custom_options_option_default', 'option_id = ' . $id);
    }
    ...
    private function _uploadImage($keyFile, $optionId, $valueId = false, $value = array()) {
        ...
        if ($imageChange) {
            ...
            foreach ($imageDelete as $optionTypeImageId) {
                $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_type_image', array('image_file'))->where('option_type_image_id = ' . intval($optionTypeImageId));
                ...
                $connection->delete($tablePrefix . 'mageworx_custom_options_option_type_image', 'option_type_image_id = ' . intval($optionTypeImageId));
            }
            ...
            foreach ($imageSort as $sort=>$optionTypeImageId) {
                ...
                $connection->update($tablePrefix . 'mageworx_custom_options_option_type_image', $data, 'option_type_image_id = '.intval($optionTypeImageId));
            }
        }        
        ...
        foreach ($keyFileArr as $index=>$keyFile) {
            if (isset($_FILES[$keyFile]['name']) && $_FILES[$keyFile]['name'] != '') {
                try {
                    ...
                    $saveResult = $uploader->save(Mage::helper('mageworx_customoptions')->getCustomOptionsPath(false, $optionId, $valueId, $_FILES[$keyFile]['name']));
                    ...
                    if ($saveResult && isset($saveResult['file'])) {
                        if ($valueId && !$isUpdate) {
                            ...
                            $connection->insert($tablePrefix . 'mageworx_custom_options_option_type_image', $data);
                        }
                        $result = true;
                    }
                } catch (Exception $e) {
    ...
    public function saveOptions() {
        ...
        $relation = Mage::getSingleton('mageworx_customoptions/relation');
        ...
        foreach ($options as $key => $option) {
            ...
            if ($helper->isSkuNameLinkingEnabled()) {
                $values = $option['values'];

                if (count($values) && $this->getGroupByType($option['type'])==self::OPTION_GROUP_SELECT) {
                    foreach ($values as $vkey => $value) {
                        if ($value['sku']) {
                            $titleBySku = $helper->getProductNameBySku($value['sku'], $storeId);
                            if ($titleBySku && $titleBySku!='') {
                                $options[$key]['values'][$vkey]['title'] = $titleBySku;
                            }
                        }
                    }
                } else {
                    if ($option['sku']) {
                        $titleBySku = $helper->getProductNameBySku($option['sku'], $storeId);
                        if ($titleBySku && $titleBySku!='') {
        ...
        if ($productId && isset($post['affect_product_custom_options'])) {
            ...
            $groupModel = Mage::getSingleton('mageworx_customoptions/group');
                            ...
                            if (isset($value['specials']) && is_array($value['specials'])) {
                                ...
                                foreach ($value['specials'] as $special) {
                                    if ($special['is_delete']=='1' || isset($specials[$special['customer_group_id']])) {
                                        if ($special['special_price_id']>0) $connection->delete($tablePrefix . 'mageworx_custom_options_option_type_special_price', 'option_type_special_price_id = ' . intval($special['special_price_id']));
                                        continue;
                                    }
                            ...
                            if (isset($value['tiers']) && is_array($value['tiers'])) {
                                ...
                                foreach ($value['tiers'] as $tier) {
                                    ...
                                    if ($tier['is_delete']=='1' || isset($tiers[$uniqKey])) {
                                        if ($tier['tier_price_id']>0) $connection->delete($tablePrefix . 'mageworx_custom_options_option_type_tier_price', 'option_type_tier_price_id = ' . intval($tier['tier_price_id']));
                                        continue;
                                    }
                                    $tiers[$uniqKey] = $tier;
                                }
    ...
    /**
     * @param MageWorx_CustomOptions_Model_Mysql4_Product_Option_Collection $collection
     * @param MageWorx_CustomOptions_Model_Catalog_Product_Option $item
     * @param $key
     * @return $this|bool
     */
    protected function filterCustomerGroups($collection, $item, $key)
    {
        $helper = Mage::helper('mageworx_customoptions');
        if (!$helper->isCustomerGroupsEnabled()) {
            return false;
     ...
     * @return $this|bool
     */
    protected function filterStoreViews($collection, $item, $key)
    {
        $helper = Mage::helper('mageworx_customoptions');
        if (!$helper->isStoreViewsEnabled()) {
            return false;
        ...
        if ($storeViewId == '0') {
            ...
            $storeViewId = $sessionQuote->getStore()->getId();
        }
     ...
     * @return bool
     */
    protected function filterViewMode($collection, $item, $key)
    {
        $isAdminOption = $item->getViewMode() == self::OPTION_VIEWMODE_BACKEND || $item->getViewMode() == self::OPTION_VIEWMODE_ADMINONLY;
        $isDisabledOption = $item->getViewMode() !== null && $item->getViewMode() == self::OPTION_VIEWMODE_DISABLE;

        if ($isDisabledOption || (!Mage::app()->getStore()->isAdmin() && $isAdminOption)) {
            $collection->removeItemByKey($key);
            return true;
        }
        return false;
     ...
     * @return $this
     */
    protected function filterOptionCollection($collection)
    {
        foreach($collection as $key => $item) {
            $this->filterViewMode($collection, $item, $key);
            $this->filterCustomerGroups($collection, $item, $key);
            $this->filterStoreViews($collection, $item, $key);
        }
     ...
     * @return bool
     */
    protected function hasRequireOptions($collection)
    {
        foreach($collection as $item) {
            if ($item->getIsRequire(true)) {
                return true;
            }
        }
        return false;
    ...
    public function getProductOptionCollection(Mage_Catalog_Model_Product $product) {
        ...
        if (!$isProductEditPage) {
            $this->filterOptionCollection($collection);

            if (!$this->hasRequireOptions($collection)) {
            ...
            if ($product->getRequiredOptions()) {
                if ($helper->isInventoryEnabled() && ($helper->getOutOfStockOptions()==1 || $helper->isSetProductOutOfStock())) {
                    ...
                    foreach ($collection as $option) {
                        ...
                        $outOfStockFlag = true;
                        foreach ($option->getValues() as $value) {
                            if ($this->isOptionValueInStock($value, $product, $collection)) {
    ...
    private function isOptionValueInStock($value, $product, $optionsCollection)
    {
        $helper = Mage::helper('mageworx_customoptions');
        ...
        if ($customoptionsQty <= 0 && $customoptionsQty !== '') {
            return false;
        }

        if ($isDependentEnabled && !$this->checkDependentInventory($optionsCollection, $value, $product)) {
            return false;
        }
    ...
    public function getOptionValueTierPrices($optionTypePriceId) {             
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
        $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_type_tier_price')->where('option_type_price_id = ' . intval($optionTypePriceId));
        $tiers = $connection->fetchAll($select);
        if ($tiers) usort($tiers, array(Mage::helper('mageworx_customoptions'), '_sortPrices'));
        return $tiers;
    }
    ...
    public function getOptionValueSpecialPrices($optionTypePriceId) {             
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
        $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_type_special_price')->where('option_type_price_id = ' . intval($optionTypePriceId));
        $specials = $connection->fetchAll($select);
        if ($specials) usort($specials, array(Mage::helper('mageworx_customoptions'), '_sortPrices'));
        return $specials;
    }
    ...
    public function getOptionValueImages($optionTypeId) {             
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
        $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_option_type_image')->where('option_type_id = ' . intval($optionTypeId))->order('sort_order');
        ...
        if ($oldProductId>0 && $newProductId>0) {
            ...
            if ($newOptions) {
                foreach ($newOptions as $option) {
                    $connection->insert($tablePrefix . 'mageworx_custom_options_relation', array('option_id' => $option['option_id'], 'group_id' => floor((intval($option['in_group_id'])-1)/65535), 'product_id' => $newProductId));
                }
            }
        }
    ...
    public function getIsRequire($isProductPage = false) {
        ...
        if (($this->getViewMode()== self::OPTION_VIEWMODE_VISIBLE || (Mage::app()->getStore()->isAdmin() && $this->getViewMode()== self::OPTION_VIEWMODE_BACKEND)) && !$this->getIsDependent()) {
    ...
    public function isContainOptionsPercentPriceType()
    {
        $values = Mage::getSingleton('catalog/product_option_value')->getValuesCollection($this);
        foreach ($values as $value) {
            if ($value->getPriceType() == self::PRICE_TYPE_OPTIONS_PERCENT) {
                return true;
            }
        }
        return false;
    ...
    public function isContainFixedPerCharPriceType()
    {
        if ($this->getPriceType() == self::PRICE_TYPE_FIXED_PER_CHARACTER) {
            return true;
        }
        return false;
    }
