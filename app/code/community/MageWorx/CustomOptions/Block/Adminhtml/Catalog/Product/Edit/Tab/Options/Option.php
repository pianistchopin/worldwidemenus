...
class MageWorx_CustomOptions_Block_Adminhtml_Catalog_Product_Edit_Tab_Options_Option extends MageWorx_CustomOptions_Block_Adminhtml_Catalog_Product_Edit_Tab_Options_Option_Abstract {

    public function __construct() {
        parent::__construct();
        if (!Mage::helper('mageworx_customoptions')->isEnabled()) return $this;
        $this->setTemplate('mageworx/customoptions/catalog-product-edit-options-option.phtml');
    }
    ...
    protected function _prepareLayout() {
        ...
        $this->setChild('copy_option_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                    'label' => Mage::helper('mageworx_customoptions')->__('Copy'),
                    'class' => 'add',
    ...
    public function getOptionValues() {                
        ...
        if (!$this->_values) {
            ...
            foreach ($optionsCollection as $option) {
                ...
                $value['show_swatch_title'] = $option->getShowSwatchTitle();
                ...
                $select = $connection->select()->from($tablePrefix . 'mageworx_custom_options_relation')->where('option_id = ' . $option->getOptionId());
    ...
    public function getShowSwatchTitleSelectHtml() {
        $select = $this->getLayout()->createBlock('adminhtml/html_select')
            ->setData(array(
                'id' => $this->getFieldId() . '_{{id}}_show_swatch_title',
                'class' => 'select'
            ))
            ->setName($this->getFieldName() . '[{{id}}][show_swatch_title]')
            ->setOptions(Mage::getSingleton('adminhtml/system_config_source_yesno')->toOptionArray());
        $select->setOptions(array_reverse($select->getOptions()));
        return $select->getHtml();
    }
    ...
    public function getViewModeSelectHtml() {        
        $select = $this->getLayout()->createBlock('adminhtml/html_select')
                ...
                ->setOptions(Mage::getSingleton('mageworx_customoptions/system_config_source_view_mode')->toOptionArray());
    ...
    public function getDependentSelectHtml() {        
        $select = $this->getLayout()->createBlock('adminhtml/html_select')
                ...
                ->setOptions(Mage::getSingleton('mageworx_customoptions/system_config_source_dependency_mode')->toOptionArray());
        $select->setOptions($select->getOptions());
        return $select->getHtml();
    }
    ...
    public function getSkuPolicySelectHtml() {
        ...
        if (method_exists($this, 'getTemplateData')) {
            ...
            $productSkuPolicy = Mage::helper('mageworx_customoptions')->getProductSkuPolicy($this->getProduct());
        }
        ...
        $select = $this->getLayout()->createBlock('adminhtml/html_select')
                ...
                ->setOptions(Mage::getSingleton('mageworx_customoptions/system_config_source_sku_policy')->toOptionArray($productSkuPolicy==3?3:2));
        return $select->getHtml();
    }
    ...
    public function getImageModeSelectHtml() {
        $select = $this->getLayout()->createBlock('adminhtml/html_select')
                ...
                ->setOptions(Mage::getSingleton('mageworx_customoptions/system_config_source_image_mode')->toOptionArray());
        $select->setOptions($select->getOptions());
        return $select->getHtml();
    }
    
    public function getDescriptionHtml() {
        $editMode = Mage::helper('mageworx_customoptions')->getDescriptionEditMode();
        if ($editMode==3) {
    ...
    public function getPriceValue($value, $type) {
        ...
        if ($type == MageWorx_CustomOptions_Model_Catalog_Product_Option::PRICE_TYPE_OPTIONS_PERCENT || $type == MageWorx_CustomOptions_Model_Catalog_Product_Option::PRICE_TYPE_FIXED_PER_CHARACTER) {
