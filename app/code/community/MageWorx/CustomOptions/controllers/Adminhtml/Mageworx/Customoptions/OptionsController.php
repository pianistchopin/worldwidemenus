...
class MageWorx_CustomOptions_Adminhtml_Mageworx_Customoptions_OptionsController extends Mage_Adminhtml_Controller_Action {
    ...
    public function editAction() {
        ...
        $model = Mage::getModel('mageworx_customoptions/group')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            ->_setActiveMenu('catalog/customoptions')
            ->_addBreadcrumb($this->__('APO'), $this->__('Manage Templates'))
            ->_addContent($this->getLayout()->createBlock('mageworx_customoptions/adminhtml_options_edit'))
            ->_addLeft($this->getLayout()->createBlock('adminhtml/store_switcher'))
            ->_addLeft($this->getLayout()->createBlock('mageworx_customoptions/adminhtml_options_edit_tabs'));
    ...
    public function duplicateAction() {
        ...
        try {
            $group = Mage::getSingleton('mageworx_customoptions/group')->load($id);
        ...
        if ($data) {
            ...
            try {
                ...
                if ($id) {
                    ...
                    $group = Mage::getSingleton('mageworx_customoptions/group');
                }                
                ...
                if ($storeId>0) { // if no default store
                    ...
                    // save store options
                    $groupStore = Mage::getSingleton('mageworx_customoptions/group_store')->loadByGroupAndStore($id, $storeId);
...
//                    exit;
                    ...
                    $groupStoreCollection = Mage::getResourceModel('mageworx_customoptions/group_store_collection')->addFieldToFilter('group_id', $id);
    ...
    public function productGridAction() {
        ...
        $this->getResponse()->setBody(
                $this->getLayout()->createBlock('mageworx_customoptions/adminhtml_options_edit_tab_product')->toHtml()
        );
    }
    ...
    public function deleteAction() {
        ...
        if ($id > 0) {
            try {
                $model = Mage::getModel('mageworx_customoptions/group');
                $model->load($id);
    ...
    public function massStatusAction() {
        ...
        if (!is_array($ids)) {
            ...
            try {
                ...
                $model = Mage::getSingleton('mageworx_customoptions/group');
                if (isset($ids) && is_array($ids)) {
                    foreach ($ids as $id) {
                        $model->load($id)
    ...
    public function exportAction() {
        ...
        if (!is_array($ids)) {
            ...
            foreach ($ids as $id) {
                ...
                $stores = array();
                $storeCollection = Mage::getResourceModel('mageworx_customoptions/group_store_collection')->addFieldToFilter('group_id', $id);
                    ...
                    foreach($data as $template) {
                        try {
                            ...
                            if ($model->getId()==$id) {
                                ...
                                $connection->insert($tablePrefix . 'mageworx_custom_options_group', $template);
                                
                                if ($stores && is_array($stores)) {
                                    foreach ($stores as $store) {
                                        $connection->insert($tablePrefix . 'mageworx_custom_options_group_store', $store);
                                    }
                                }
